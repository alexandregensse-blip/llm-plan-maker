# plan-suite v17 — Blind Confrontation, Compelled Navigation, Honest Diversity
...
3–5 past planning failures through it and check which gates would have caught
them, at what cost. Revise or cut sections that do not earn their keep.
...
> load-bearing commitment that closes options must be validated by the cheapest
> source that can settle it — the environment first, the user second, a blind
> isolated model third, nothing last. Multi-agent compute is spent on exactly
> three things: **innovation** (framings nobody has proposed), **blind
> confirmation** (independent verification of a claim), and **option
...
1. **State over prose.** Maintain a living state (§2), not a static document,
   until one is actually requested or required for handoff.
...
3. **Blind or nothing.** An agent that saw another agent's conclusion before
   forming its own is not an independent confirmation. Label independence
   honestly, never upgrade the label.
4. **Containment before confidence.** An irreversible or costly action ships
   only with a named revert path, or behind an explicit user gate.
5. **Agents are not evidence.** Only the environment (tool, test, inspection,
   source) or the user verifies a fact. A model's own output — including
   another instance's — becomes evidence only when it performed an
   independent, inspectable check.
6. **No invented steps past the fog of war.** A step whose shape depends on an
   earlier step's outcome is not planned in advance; it is named as an open
   question.
7. **Silence is not authorization.** For a preference-dependent, irreversible
   trade-off, no reply from the user is never treated as approval, unless that
   decision class was explicitly delegated in advance.
8. **Token economy is a design constraint.** Every section, every spawn, every
   evidence check must earn its cost. Default to the cheapest thing that
...
models rubber-stamping each other · mode collapse under identical briefs ·
hiding uncertainty to look complete · planning past the fog of war · skipping
containment on irreversible steps · guessing a user's preference · running
heavy multi-agent machinery on questions that needed a direct answer.
...
| **L2** | A commitment is load-bearing, OR material unknowns remain, OR the environment is unfamiliar | Full commitment ledger (§3) + evidence (§4) + **Explorer pass + blind confrontation where §5's trigger fires** (§5–§6) + resolution (§7). |
...
`depth=L2, territory=frontier because: <observable signals>`.
...
preemptively. This block is reused everywhere below — inside the Frontier
Loop, at the head of a written Plan, and in a handoff.
...
- **Target must be falsifiable.** "Improve security" is not a target. "Test X
  shows access paths closed under threat model T" is.
- **Invariants ≠ preferences.** "Do not delete production data" is an
  invariant. "Prefer managed services" is a preference — record it, and only
  escalate it to the user when it actually decides a commitment (§7.2, §8).
- **"Verified" means you looked.** Tool output, inspection, a direct user
  statement. Never training data, prior-session memory, or another agent's
  assertion.
- **Evidence has a shelf life.** A load-bearing fact carries an implicit TTL
  of "this session" unless stated otherwise. Re-check it at execution time
  if time has passed or the environment could have changed; a fact that
  turned out stale is a replan trigger (§10), not a silent carry-over.
...
actually closes options. Everything else is glue.
...
an email, notify a third party); adopting an architecture or dependency that
forecloses alternatives; taking on an external dependency (a person, a
...
resolved (creating a directory implied by an already-approved migration, the
health check that validates a step you already decided to take). Tag these
`derived` and let them inherit their parent's justification — do not re-open
them.
...
At L1, skip the schema; carry the commitment as one sentence in your head or
the Living State.
...
- **No, because a fact is missing** → this is not a choice, it's a gap.
  Convert to a discovery action (§4) or an experiment. Do not generate
  candidates to paper over missing information.
- **No, because ≥2 options genuinely survive the evidence** → candidates may
  be warranted. Continue to §5.
...
Treat every candidate spawn as something you have to justify, not something
you default into.
...
inspection, test, or recomputable calculation actually exists for it.
Everything else stays an explicit assumption.
...
**gap**, and branch, gate, or escalate — do not keep spending tokens chasing
certainty the budget already says isn't worth it.
...
> **Trigger, not quota.** Spawn agents only when at least one of these holds.
> Otherwise: skip the section and record in the ledger that it was skipped
> and why. A spawn without a distinct question, information boundary, or
> method to test is theater — it inflates apparent confidence without adding
> it.

Three legitimate reasons to spawn:

1. **Innovation.** The current framing might be too narrow; nobody has
   proposed a genuinely different approach yet. → **Explorer** (§5.2).
2. **Blind confirmation.** A load-bearing claim needs an independent
   verification that does not inherit the claim's case. → **Verifier** (§5.2).
3. **Contested choice.** ≥2 options survive the compulsion check, the
   commitment is load-bearing, and the choice is at least weakly reversible
   or the consequence is material enough that a wrong pick is expensive. →
   **Candidate** (§5.2), followed by **Risk attacker** (§6).
...
### 5.1 The Explorer pass — mandatory at L2/L3 before framing is frozen

The single highest-value multi-agent move this skill prescribes. Before you
commit to a framing, spawn (or simulate in `staged` mode) an Explorer whose
only job is to challenge the framing itself:

- Are there materially different approaches to this objective?
- Is the current problem statement assuming something that could be false?
...
The Explorer's output is not a candidate; it is input to whether the
**compulsion check even applies to the framing you were about to use**. Skip
this at your own risk: the most expensive planning failures are framing
failures, not choice failures.

### 5.2 Roles, and when each earns its spawn

| Role | Purpose | Spawn when |
|---|---|---|
| **Explorer** | Reframes the problem, surfaces options nobody proposed yet, hunts hidden dependencies | Before the framing is frozen, at L2/L3 |
...
Prompts for each role are in Appendix A. All of them are given the facts and
the question — never the requester's persuasive case for a particular answer.
...
Temperature is not independence. Self-critique is not a review. A same-model
"reviewer" in a fresh context is *weakly* independent — useful for surfacing
failure modes, useless as corroboration — and that label never gets upgraded
because the output sounds confident.

### 5.4 Candidate generation — diversity of method, not of wording
...
If the runtime cannot actually spawn that many isolated contexts, fall back
to `staged` mode and label it as such — never claim independence you didn't
achieve.
...
Each candidate must state: the option (one sentence); rationale (3–5
bullets); explicit assumptions; rejected alternatives *and why they remain
viable*; dependencies; its two most likely failure modes; a validation plan;
what would change the recommendation. A candidate that silently fills an
...
### 5.6 Stopping rule — when to stop spawning

Stop spawning when any of these holds:

- The decision is stable: no plausible additional agent output would change
  the commitment.
- Remaining uncertainty is safely held by a gate, a monitor, or a reversible
  experiment.
- The marginal agent would only make the current story sound more convincing
  — not change what is done.
- The evidence budget for this commitment is exhausted (§4.1).

Ask before every spawn: **"Could this context change what we do, or merely
make it sound better?"** Only the first justifies the tokens.

---

## 6. Adversarial pass — attack, don't review

Runs independently of generation. Same rule as above: give the attacker the
proposed action and the facts, not the case for it.
...
L2: one lens, chosen by what actually worries you about this specific
commitment. L3: at least two distinct lenses.
...
Never pick a "winning candidate's whole plan." Resolve each commitment on its
own merits and reassemble.
...
- **Correlated-error check.** Before treating agreement as confirmation,
  ask: did the candidates use genuinely different evidence paths, or do they
  share an assumption, a source, or a model family? If they share one, that
  shared point is still unverified, however many agents repeated it.
- **No silent synthesis.** Any action in the final plan that no candidate
  ...
  derivation isn't obvious, it's a new commitment — run §3.1–§7 on it, don't
  wave it through because assembly is almost done.
...
available. Use them deliberately, not as a last resort after guessing fails.

**When to ask:** the choice cannot be settled by evidence or constraint; it's
materially consequential; and it's a preference rather than a fact. Ask once,
...
- Silence is **not** authorization for an irreversible, preference-dependent
  trade-off — unless the user has previously and explicitly delegated that
  class of decision.
...
- Cap questions per phase at 2. Never ask about what is cheaper to verify by
...
now`, determine the new `Next Step`. Doesn't match? **Stop.** Do not compound
errors — revert if the Rollback Plan is still safe, re-evaluate `Verified
now`, and only then decide the next step.
...
output ends the sequence there. What follows is a named open question, not an
invented step — if you're tempted to keep going, delete Steps 3+ and drop
into the Frontier Loop instead.
...
be visible together before execution starts (L2/L3, multi-commitment work, or
explicit request).
...
past the fog — cut back to the next few material steps and drop the rest into
open questions.
...
- **Mismatch** → **stop**. Inspect the actual state with a discovery action.
  Roll back if Containment is defined and still safe. Identify the false
  premise. Replan the smallest affected subtree — rebuild the whole Living
  State only if the Target or an invariant is now in question.
...
local failures mean the world model is wrong, not the step · the target or a
...
replan triggers. Nothing load-bearing lives in memory alone — if it mattered
enough to act on, it's written down.

---

## 11. Planning debt — ship at 80% when the rest is safe

A plan must be executable and recoverable, not exhaustively complete. If the
missing 20% is held behind a gate, a monitor, or a cheap reversible
...
Never silently drop coverage to fit a budget — an omission the reader can't
see is worse than one flagged and shipped.
...
explanation, or a trivial reversible action · no commitment in the request is
load-bearing · the user explicitly wants speed and every step is reversible ·
running this costs more than the plan is worth · the environment offers no
observation to validate against — say so, don't plan against your own
imagination.

A skill that never says "I am not the right tool for this" is dangerous. This
one does — and that judgment happens at Triage (§1), every time, not just at
...
- [ ] Every "verified" fact comes from observation with a locator, not memory
      or agreement.
- [ ] The compulsion check ran on every commitment before any candidate
      generation — most should show zero candidates.
- [ ] The Explorer pass ran at L2/L3 before the framing was frozen.
...
> Know your territory before you plan across it; let compulsion, not habit,
> decide where you spend deliberation; spend multi-agent compute only on
> innovation, blind confirmation, and genuine option diversity; confront the
> commitments where a real choice survives, with agents that are honestly
> independent and attacks that don't defend; contain what you cannot undo;
> verify with the environment rather than your own model of it; and let what
> execution actually observes — not what you assumed — decide the next step.

---

## Appendix A — Role prompts

### A.1 Explorer (before the framing is frozen)
```
